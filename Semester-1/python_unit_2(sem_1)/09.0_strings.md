...
> **Start is inside, Stop is outside.**

---

## 4.1 Understanding Indexes Before Slicing
...
Before solving a slicing question, first identify the indexes.

### Three Questions to Ask

Whenever you see a slice, ask:

```text
1. START → Where should I start?
2. STOP  → Where should I stop?
3. STEP  → How many positions should I jump?
```

---

## 4.2 Slicing With Start and Stop
...
Index `4` is the stopping point, so it is not included.
...
> **Start missing → start from the beginning.**
...
> **Stop missing → continue to the end.**

---

## 4.5 Omitting Both Start and Stop
...
Slicing can have a third value called `step`.
...
STEP  → How many positions do I jump?
```

### What Is Step?

`step` tells Python how many positions to move after selecting a character.
...
step = 2 → move two positions at a time
step = 3 → move three positions at a time
```
...
If `step` is not written, Python uses `1`.
...
> **Positive step → move from left to right.**

---

## 4.9 Different Positive Steps
...
# 4.10 Negative Indexing With Slicing
...
> Start at index `-3` and continue to the end.
...
Index `1` is the stopping point, so it is excluded.
...
# 4.12 Reverse a String

One of the most useful slicing shortcuts is:
...
Python therefore starts from the end and moves toward the beginning.

---

# 4.13 Reverse With Step -2
...
Starting from index `5` and moving by `-2`:
...
> **Negative step = reverse direction + jump**

---

# 4.14 Negative Index Slicing Example
...
# 4.15 A Simple Slicing-Solving Trick ⭐

For every slicing problem, follow these steps:

### Step 1: Find START

Where should Python start?
...
### Step 3: Move According to STEP

Examples:

```text
step = 1  → move 1 position at a time
step = 2  → move 2 positions at a time
step = -1 → move 1 position backward
step = -2 → move 2 positions backward
```

### Step 4: Stop Before STOP
...
```text
START → Direction → Jump → Stop before STOP
```

---

# 4.16 Important Slicing Patterns

| Pattern | Meaning |
|---|---|
| `s[:]` | Complete string |
| `s[:n]` | From the beginning up to `n` (excluding `n`) |
| `s[n:]` | From index `n` to the end |
| `s[n:m]` | From index `n` to `m-1` |
| `s[::2]` | Every 2nd character |
| `s[::3]` | Every 3rd character |
| `s[::-1]` | Reverse the string |
| `s[::-2]` | Reverse direction with step `-2` |
| `s[-n:]` | Last `n` characters |
| `s[:-n]` | All except the last `n` characters |

---

# 4.17 Slicing and Out-of-Range Indexes
...
Python does not raise `IndexError` just because a slice boundary is outside the string.
...
> **Out-of-range slice boundaries are generally handled automatically.**

---

# 4.18 Wrong Direction in Slicing

Some slices look incorrect but do not produce an error.
...
The default step is `+1`, so Python moves from left to right.

But the start index `5` is already to the right of the stop index `2`.

No characters can be selected in that direction.
...
A negative step means move from right to left, but stop `5` is to the right.

### Important Point

> **A wrong-direction slice usually returns an empty string `''`; it does not raise an error.**

---

# 4.19 Step Cannot Be Zero ❌

One slicing value is actually invalid:
...
`step` represents movement or jumping.

```text
step = 2  → move 2 positions
step = -1 → move 1 position backward
step = 0  → no movement
```

Python does not allow a zero step.

### Golden Rule

> **Step can be positive or negative, but it can never be zero.**

---

# 4.20 Integer Is Not Sliceable

Slicing works with strings and other sequence types, but an integer is not sliceable.
...
It produces a `TypeError` because an integer does not support indexing or slicing.

If you need to slice the digits, represent the number as a string:
...
# 4.21 Common Beginner Mistakes in Slicing
...
## Mistake 2: Forgetting That Step Controls Direction
...
## Mistake 3: Thinking Every Out-of-Range Slice Is an Error
...
Python simply stops at the end of the string.
...
## Mistake 5: Confusing Indexing and Slicing
...
does not normally give `IndexError`.

---

# 4.22 Real-Life Slicing Examples
...
Special cases:
--------------------------------
Out-of-range slice → Usually no error
Wrong direction     → Usually ''
step = 0            → ValueError
Invalid index       → IndexError
Integer slicing     → TypeError
```
