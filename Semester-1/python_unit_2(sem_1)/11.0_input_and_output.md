...
When we use `print()` to display text, sometimes we need special characters or formatting inside a string.

Python provides **escape sequences** for this purpose.
...
| `\t` | Tab / horizontal space | `print("Name:\tRahul")` |
| `\\` | Backslash | `print("C:\\Python")` |
| `\'` | Single quote | `print('It\'s Python')` |
| `\"` | Double quote | `print("He said \"Hello\"")` |
| `\r` | Carriage return | `print("Hello\rHi")` |
| `\b` | Backspace | `print("Hell\bo")` |
...
This is useful for making simple output easier to read.

---

## 9.3 Backslash: `\\`

A backslash is itself a special character in Python strings.

To display one backslash, use `\\`.
...
If a string is enclosed in single quotes and we want to display a single quote inside it, we can use `\'`.
...
Python may interpret the second `'` as the end of the string.

---

## 9.5 Double Quote: `\"`

If a string is enclosed in double quotes and we want to display double quotes inside it, we can use `\"`.
...
## 9.6 Escape Sequences and `print()`

Escape sequences are especially useful when creating formatted output.
...
> Escape sequences control how characters are displayed inside a string.

---

## 9.7 Escape Sequence vs `print()` `end`

Both can be used to control lines, but they work differently.
...
Similarly, `end` can prevent `print()` from moving to a new line:
...
end  → controls what print() adds after its output
```
...
0. Escape sequences use `\` inside strings to represent special characters or formatting.
0. `\n` creates a new line.
0. `\t` inserts a tab.
0. `\\` represents a backslash.
0. `\'` represents a single quote.
0. `\"` represents a double quote.
